...
1.9：登录页实际 Cookie 里有没有 DVIF。改写后的请求头、系统 Cookie 仓、网页 Cookie 仓分开记，不记值。
...
1. 巨魔打开极速版，只做注入（建议顺序）：
     ...
     BDSLoginProbe.dylib               ← 本探针 1.9（替换更早的探针）
   两个都留下。不要卸 卐解。
...
GitHub Actions：push 或 workflow_dispatch，产物名 BDSLoginProbe-1.9。
